 ...
 */
function controller($name,$path=''){
    $class  =   ( $path ? basename(ADDON_PATH).'\\'.$path : MODULE_NAME ).'\\'.C_LAYER;
    $array  =   explode('/',$name);
    foreach($array as $name){
        $class  .=   '\\'.parse_name($name, 1);
    }
...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    if($url) {
        if(0=== strpos($url,'/')) {// 定义路由
            ...
            if($urlCase) {
                $var[VAR_CONTROLLER]   =   parse_name($var[VAR_CONTROLLER]);
            }
            $module =   '';
            
            if(!empty($path)) {
                $var[VAR_MODULE]    =   implode($depr,$path);
            }else{
                if(C('MULTI_MODULE')) {
                    if(MODULE_NAME != DEFAULT_MODULE || !C('MODULE_ALLOW_LIST')){
                        $var[VAR_MODULE]=   MODULE_NAME;
                    }
                }
            }
            if($maps = C('URL_MODULE_MAP')) {
                if($_module = array_search(strtolower($var[VAR_MODULE]),$maps)){
                    $var[VAR_MODULE] = $_module;
                }
            }
            if(isset($var[VAR_MODULE])){
                $module =   $var[VAR_MODULE];
                unset($var[VAR_MODULE]);
            }
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        $url        =   __APP__.'?'.VAR_MODULE."={$module}&".http_build_query(array_reverse($var));
        if($urlCase){
            $url    =   strtolower($url);
        }        
        if(!empty($vars)) {
